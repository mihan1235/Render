...
GLuint kr::prepare_texture(const char* name) {
...
GLuint kr::generateAttachmentTexture(GLboolean depth, GLboolean stencil) {
	// What enum to use?
	...
	if (!depth && !stencil) {
		glTexImage2D(GL_TEXTURE_2D, 0, attachment_type, Core::get_width(), Core::get_height(),
			         0, attachment_type, GL_UNSIGNED_BYTE, NULL);
	}
	else {
		// Using both a stencil and depth test, needs special format arguments
		glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH24_STENCIL8, Core::get_width(), Core::get_height(),
